    ...
    <section className="container-shell">
      <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
        <div className="min-w-0 p-2 sm:p-4">
          ...
          <p className="mt-5 text-base leading-8 text-(--muted) sm:text-lg">{content.description}</p>
        </div>
        <div ref={cardGridRef} className="grid min-w-0 gap-4 xl:grid-cols-3">
          {content.items.slice(0, 3).map((item, index) => {
            const backgroundImage = TESTIMONIAL_BACKGROUND_IMAGES[index % TESTIMONIAL_BACKGROUND_IMAGES.length];
            ...
            return (
              <article
                ...
                className={cn(
                  ...
                  isCardGridVisible ? "translate-y-0 scale-100 opacity-100" : "translate-y-10 scale-[0.98] opacity-0",
                )}
              ...
              >
                ...
                <div className="relative z-10 flex h-full flex-col justify-end">
                  <p className="text-xs tracking-[0.2em] text-white/65 uppercase">Partner perspective</p>
                  ...
                  <div className="mt-3 max-h-0 translate-y-2 overflow-hidden opacity-0 transition-all duration-500 ease-out group-hover:max-h-56 group-hover:translate-y-0 group-hover:opacity-100">
                    <p className="border-t border-white/25 pt-4 text-sm leading-7 text-white/85">“{item.quote}”</p>
