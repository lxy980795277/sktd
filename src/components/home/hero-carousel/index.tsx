  ...
  return (
    <section className="container-shell pt-8 sm:pt-10 lg:pt-12">
      <div className="grid gap-6 lg:grid-cols-[1.08fr_0.92fr]">
        <div className="glass-card rounded-[32px] border border-(--line) p-6 shadow-[0_24px_80px_rgba(31,29,25,0.08)] sm:p-8 lg:p-10">
          ...
          <div className="mt-8 flex flex-wrap items-center gap-3 text-sm text-(--muted)">
            <span className="rounded-full bg-(--surface) px-4 py-2 font-medium text-foreground">
              {activeSlide.note}
            </span>
            <span>
          ...
          <div className="mt-8 grid gap-3 sm:grid-cols-3">
            {activeSlide.highlights.map((item) => {
              return (
                <div
                  key={item}
                  className="rounded-[24px] border border-(--line) bg-white/72 px-4 py-4 text-sm leading-7 text-foreground"
                >
                  {item}
                </div>
              );
        ...
        <div className="relative overflow-hidden rounded-[32px] border border-(--line) bg-[#e8dccd] shadow-[0_24px_80px_rgba(31,29,25,0.08)]">
          <div className="absolute inset-x-0 top-0 z-10 flex items-center justify-between p-5">
            ...
            <div className="flex items-center gap-2">
              <button
                type="button"
                className="inline-flex h-11 w-11 items-center justify-center rounded-full bg-white/90 text-foreground shadow-sm"
