...
type NotFoundViewProps = {
  content: CommonContent["notFound"];
  homeHref: string;
};

export function NotFoundView({ content, homeHref }: NotFoundViewProps): React.JSX.Element {
  return (
    <main className="flex min-h-screen items-center justify-center px-6">
      <div className="glass-card max-w-xl rounded-[32px] border border-(--line) px-8 py-10 text-center shadow-[0_24px_80px_rgba(31,29,25,0.08)]">
        ...
        <h1 className="section-title text-4xl font-semibold sm:text-5xl">{content.title}</h1>
        <p className="mt-4 text-sm leading-7 text-(--muted) sm:text-base">{content.description}</p>
        <Link
          href={homeHref}
